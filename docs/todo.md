...
**Created:** 2026-05-15  
**Last updated:** 2026-07-08 (M9-022 done: wired the never-called set_park_position() into HOME confirmation so "Stop safely" no longer faults with "home the mount first")
...
- [x] M9-022 Real hardware: "Stop safely" faulted with `:hP# rejected by OnStep — home the mount first to establish the park position, then park", even right after a successful HOME confirmation `[P0 · Hardware · Source: user report 2026-07-08]`
  ...
  - *Done:* `ObservingService._run_home()` now calls `deps.mount.set_park_position()` right after confirming `AT_HOME` (guarded by `if at_home:` — never attempted if home wasn't actually reached), matching the adapter's own documented intent. Best-effort: a rejection (return `False` or a raised `OnStepSafetyError`, e.g. a stale safety lock) is logged and recorded in `detail["home"]["park_position_set"]` but does **not** invalidate `g2_home_confirmed` — G2 is about physically reaching home, not park bookkeeping; a genuine park failure will still surface on its own via the later "Stop safely" attempt.
  - `adapters/mock/mount.py`'s `MockMount` gained a `set_park_position()` returning `True` (previously inherited `MountPort`'s `False` default, silently). `tests/conftest.py`'s shared `mount_mock` fixture now pre-configures `set_park_position.return_value = True`, matching its other mount-method defaults.
  - Tests: updated `TestConfirmHome`'s happy-path assertion for the new `detail["home"]` shape plus a `set_park_position.assert_called_once()` check; new not-called assertion in the not-reached-AT_HOME test; new `test_set_park_position_failure_does_not_invalidate_home` covering the soft-failure path. Full sweep: 3925 passed, same 4 pre-existing/unrelated `test_get_sync_status.py` failures, 0 new regressions. Verified end-to-end via direct API calls against the mock-adapter server: `START_HOME` → `detail.home.park_position_set: true` → `STOP_SAFELY` → reaches `PARKED_SAFE` with `g8_safe_stop_possible: true`.
