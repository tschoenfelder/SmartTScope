...
- [OnStepAdapter external module](../SYNC.md) — External pip package tschoenfelder/OnStepAdapter (currently v0.3.0), the sole intended way to connect to mount/focuser — SmartTScope must not duplicate this code (guardrail restated 2026-07-09). **Process rule: never edit `OnStepMount`/`OnStepClient`/etc. directly — flag fixes to the user in chat and wait; canonical source is the published git release only, not any local checkout.** Migration plan ONS-MIGRATE-001..014 in `docs/todo.md` aims to reduce `smart_telescope/adapters/onstep/` to a ≤30-line MountPort shim; active SYNC-OVERRIDEs, upstream contribution requirements (REQ-1, REQ-2, REQ-ST-001..008), and upgrade procedure in SYNC.md.
...
- **M9-022 corrected (twice):** the original fix (auto-calling `mount.set_park_position()` from `_run_home()` after every HOME confirmation) was itself a regression of a 2026-06-14 fix that explicitly removed exactly this auto-set-park pattern for silently overwriting the user's configured EEPROM park position — reverted. A follow-up explicit "Set Park Position" UI (added as the "proper" replacement) was then *also* removed once the user clarified no requirement actually asks for a UI way to change park position, only that it must never happen automatically. Net: `_run_home()` doesn't touch park position at all; there is currently no way to set it through this app, by design, until a real requirement exists.
- **M9-023 done:** the mount-strip could keep showing PARKED for several seconds after HOME was confirmed, even though the Observe screen's phase panel had already moved on — `_run_home()` was missing the `deps.device_state.poll_now()` call that `_run_safe_stop()` already has after `park_sequence()`, to force-refresh the independently-polled mount-status cache. Added; verified `/api/mount/status` now reflects `at_home` with zero delay after `START_HOME`.
- **M9-024 done:** the *actual* root cause of the repeated "why does it say confirm home while parked" confusion across M9-021/022/023 was the button label itself — "Confirm HOME position" implied an already-true state being acknowledged, when clicking it actually performs the unpark+slew-to-home action from PARKED. Relabeled to "Home the mount"; the two-step start/accept structure underneath (M9-007/M9-016) was already correct.
- **M9-025 done:** `home_sequence()` now disables tracking immediately and unconditionally after a successful unpark, instead of only after a subsequent `get_state()` check — some OnStep firmware auto-starts tracking on unpark itself, and the old check raced against that, risking unwanted mount movement before the home command was issued.
- **M9-026 done:** real hardware `:hP#` (park) rejection, diagnosed from the actual server log — `park_sequence()`'s post-command check compared the mount's state against a hardcoded `MountState.UNPARKED` instead of its actual pre-park state, so parking from `AT_HOME` (the guided home→park path) always falsely reported "slew started" without the mount moving, causing a retried `:hP#` that OnStep genuinely rejected. Fixed to compare against the real `pre_state`.
- **M9-027 done:** the actual reason a second `:hP#` got sent (and rejected) — `_maybe_auto_advance()` re-spawns `_run_safe_stop()` on every poll (every 2.5 s) while SAFE_STOPPING hasn't reached PARKED, blindly resending `:hP#` each time. `:hP#` is documented fire-and-forget with no known rejection case, unlike `:hR#`; resending it while a previous accepted command might still be resolving is not something OnStep's own UI ever does. `ObservingService` now tracks whether a park command was already issued and skips resending it for up to 120 s (the documented max slew time), just re-checking `device_state` instead.
