...
**Created:** 2026-05-15  
**Last updated:** 2026-07-08 (M9-022 corrected: reverted the auto-set-park-position regression, added an explicit "Set Park Position" action instead — see wiki/log.md 2026-06-14 for why automatic was already ruled out once)
...
- [x] M9-022 Real hardware: "Stop safely" faulted with `:hP# rejected by OnStep — home the mount first to establish the park position, then park", even right after a successful HOME confirmation `[P0 · Hardware · Source: user report 2026-07-08]`
  ...
  - *Corrected:* `_run_home()` no longer calls `set_park_position()` at all — reverted to the M9-021 shape (`g2_home_confirmed` from `at_home` alone, `detail["home"]` back to just `{"mount_state": ...}`). In its place: a new explicit, two-step-confirm `POST /api/mount/set_park_position` endpoint (`api/mount.py`, mirroring the existing `mount_park`'s `{confirmed: bool}` pattern) and a "Set Park Position" button in the Maintenance mount-strip button group (`static/js/mount.js`, next to Home/Unpark/Park) that shows a native `confirm()` dialog before saving — the deliberate, user-initiated action that should have existed since the 2026-06-14 removal but never did, which is the real reason `:hP#` had nothing to park to in the first place.
  - Tests: reverted `TestConfirmHome`'s M9-022-specific assertions (dropped the `set_park_position` call/detail-shape checks, restored the not-called check); new `TestMountSetParkPosition` class in `test_mount.py` (confirm-required first, success, mount-rejection → 409, `OnStepSafetyError` → 409 with reason). Full sweep: 3929 passed, same 4 pre-existing/unrelated `test_get_sync_status.py` failures, 0 new regressions. Verified via direct API calls: `START_HOME` → `detail.home` has no `park_position_set` field; `POST /api/mount/set_park_position` requires `confirmed:true` before calling `mount.set_park_position()`.
