...
**Created:** 2026-05-15  
**Last updated:** 2026-07-08 (M9-026 done: found and fixed the real bug from server logs — park_sequence() compared against a hardcoded MountState.UNPARKED instead of the actual pre-park state, falsely reporting "slew started" when parking from AT_HOME never actually moved the mount, triggering a second :hP# that OnStep genuinely rejected)
...
- [x] M9-026 Real hardware: `:hP#` (park) rejected by OnStep, mount stuck at AT_HOME through two park attempts `[P0 · Hardware/Bug · Source: user report 2026-07-08]`
  ...
  - **Still open:** *why* the mount didn't actually move/progress after the first accepted `:hP#` (a real mechanical/firmware question — possibly specific to parking directly from the mechanical-HOME route vs. from TRACKING/UNPARKED, unconfirmed) — and whether `park_sequence()` should avoid blindly re-issuing `:hP#` on a retry if a previous attempt was already accepted but not yet confirmed complete. Not fixed here; flagged for the next real-hardware test.
