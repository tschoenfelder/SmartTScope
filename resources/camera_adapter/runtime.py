...
def _build_adapters(
    ...
    main_spec = config.CAMERA_SPECS.get("main")
    ...
    camera: CameraPort
    cam_mode: str
    if main_spec and main_spec.enabled and not os.environ.get("TOUPTEK_INDEX"):
        if main_spec.backend.lower() != "native":
            _log.error(
                "Configured camera backend '%s' is not available in the MVP runtime — "
                "falling back to MockCamera. Use native, or run an explicit all-INDI profile.",
                main_spec.backend,
            )
            camera = MockCamera()
            cam_mode = "mock"
            ...
            try:
                ...
                if not camera.connect():
                    raise RuntimeError("Configured main ToupTek camera failed to connect")
                ...
                cam_mode = "real"
                _log.warning("Adapter selected: SmartTouptekCamera(role=main, model=%s)", main_spec.model or "*")
            except Exception as exc:
                _log.error(
                    "Camera init failed for model=%r: %s — falling back to MockCamera. "
                    "Check USB connection and camera config.",
                    main_spec.model, exc,
                )
                camera = MockCamera()
                cam_mode = "mock"
    ...
    def get_camera_by_role(self, role: str) -> CameraPort:
        ...
        if role in config.CAMERA_SPECS and config.CAMERA_SPECS[role].enabled:
            ...
            if role not in self._role_cameras:
                ...
                if spec.backend.lower() != "native":
                    raise HTTPException(
                        ...
                        detail=(
                            ...
                            "use a dedicated all-INDI profile for INDI."
                        ),
                    )
                from .adapters.touptek.managed import SmartTouptekCamera
                ...
                if not cam.connect():
                    raise RuntimeError(f"Camera role {role!r} failed to connect")
