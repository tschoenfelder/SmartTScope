...
- **M9-026 open:** real hardware `:hP#` (park) rejection with a park position already saved in OnStep — root cause not yet identified. Corrected the misleading error message (stopped asserting an unverified cause); awaiting the actual OnStep reply from the server log to diagnose further.
