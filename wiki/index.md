...
- **M9-022 corrected:** the original fix (auto-calling `mount.set_park_position()` from `_run_home()` after every HOME confirmation) was itself a regression of a 2026-06-14 fix that explicitly removed exactly this auto-set-park pattern for silently overwriting the user's configured EEPROM park position. Reverted; added the missing piece properly instead — a new explicit, two-step-confirm `POST /api/mount/set_park_position` endpoint plus a "Set Park Position" button (Maintenance, behind a `confirm()` dialog).
