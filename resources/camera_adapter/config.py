...
def _load_config_from_disk() -> tuple[dict, ConfigError | None, Path | None]:
    """Load the first found config file. Returns (cfg_dict, error_or_None, path)."""
    for path in _SEARCH_PATHS:
        if path.exists():
            try:
                with path.open("rb") as fh:
                    return tomllib.load(fh), None, path
            except tomllib.TOMLDecodeError as e:
                return {}, ConfigError(f"Config parse error in {path}: {e}"), path
    return {}, None, None


_cfg, _load_error, CONFIG_PATH = _load_config_from_disk()
...
def _camera_spec_from_dict(role: str, vals: dict) -> CameraSpec:
    ...
    return CameraSpec(
        ...
        backend=str(vals.get("backend", _get("hardware", "camera_backend", "native"))),
...
CAMERA_BACKEND: str = os.environ.get("SMARTTSCOPE_CAMERA_BACKEND", _get("hardware", "camera_backend", "native"))
...
def _parse_filters() -> dict[str, int]:
    section = _cfg.get("filters", {})
    result: dict[str, int] = {}
    for name, position in section.items():
        if isinstance(position, dict):
            continue
        result[str(name)] = int(position)
    return result


FILTERS: dict[str, int] = _parse_filters()
