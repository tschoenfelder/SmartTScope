...
- **Mount AT_HOME state done:** `MountState.AT_HOME` added to enum; `DeviceStateService` uses `_sticky_at_home` flag (set by `record_command("home")`, cleared by "goto"/"park"/"track") to promote UNPARKED→AT_HOME after a home slew — needed because OnStep's 'H' flag in `:GU#` clears in under 2 s (less than the 2 s poll interval); blue "Home" badge in mount tile; `mount.js` `_STATE_LABEL` maps state to display text
